...
impl StyleSource {
    ...
    #[inline]
    pub fn read<'a>(&'a self) -> RwLockReadGuard<'a, PropertyDeclarationBlock> {
        use self::StyleSource::*;
        match *self {
            Style(ref rule) => rule.block.read(),
            Declarations(ref block) => block.read(),
        }
    }
}
    ...
    fn dump<W: Write>(&self, writer: &mut W, indent: usize) {
        ...
        match self.source {
            Some(ref source) => {
                ...
                let _ = write!(writer, "  -> {:?}", source.read().declarations);
            }
            None => {
                if indent != 0 {
...
pub struct RuleTreeDeclarationsIterator<'a> {
    current: Option<(&'a StrongRuleNode, RwLockReadGuard<'a, PropertyDeclarationBlock>)>,
    index: usize,
}
